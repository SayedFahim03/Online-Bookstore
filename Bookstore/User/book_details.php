...
// to get current page
    ...
    //function for user rating badge bg
    function rating_bg($rating){

         if($rating == 5){ return 'badge-success'; }
         if($rating ==4){ return 'badge-primary'; }
        if($rating ==3){  return 'badge-info';}
        if($rating ==2) { return 'badge-warning text-white'; }
        if($rating == 1){ return 'badge-danger'; }
    }
...
//function to paginate reviews
    function paginate_reviews($page,$sorter,$order){
        ...
        $review  .= '<div class="alert alert-light border border-default" role="alert">
                    <h6 class="alert-heading"><span class="badge '.rating_bg($row['rating']).'">'.$row['rating'].'&nbsp;
                    <i class="fas fa-star" style="color:white"></i></span>&nbsp;&nbsp;'.$row['rating_title'].'</h6>
                    <p>'.$row['review'].'</p>
                    <hr>
...
//funtion to check if reviews exist for product
function check_reviews(){
    
    if(if_rating_exist()<=0)
    {
        return 'no reviews';
    }else{

        !isset($_SESSION['text']) ? $_SESSION['text'] = 'most helpful' : '';
        return 'User Reviews  '.'<span class="text-muted h6">(Showing: '.$_SESSION['text'].' )</span>';
        
    }

}

//funtion to show review sorter dropdown if review exist
function show_review_sorter(){
    ...
    if(if_rating_exist()<=0)
    {
        ...
        return '<form action="" method="post" class="float-right form-inline">
...
<div class="row">
    ...
    <div><span class="h4"><?= check_reviews() ?></span></div>
    ...
    $(document).ready(
        ...
        $('#review_sort').change(function(){
            ...
            $.ajax({
                ...
                window.location.href = "book_details.php?id=<?=$id?>&page=1";
